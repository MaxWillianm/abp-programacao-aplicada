...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
        ...
        $sfn = mb_strtolower(CKFinder_Connector_Utils_Misc::mbBasename($uploadedFile['name']));
        $sFileName = CKFinder_Connector_Utils_FileSystem::convertToFilesystemEncoding($sfn);
        ...
        if(strpos($sFilePath, "jpg") !== false && class_exists("Imagick")) {
            $img = new Imagick($sFilePath);
            $this->autoRotateImage($img);
            $img->writeImage($sFilePath);
            $img->clear();
            $img->destroy();
        }
    ...
    private function autoRotateImage(&$image)
    {
        ...
        switch($orientation) {
            case imagick::ORIENTATION_BOTTOMRIGHT:
                $image->rotateimage("#00000000", 180); // rotate 180 degrees
            break;

            case imagick::ORIENTATION_RIGHTTOP:
                $image->rotateimage("#00000000", 90); // rotate 90 degrees CW
            break;

            case imagick::ORIENTATION_LEFTBOTTOM:
                $image->rotateimage("#00000000", -90); // rotate 90 degrees CCW
            break;
        }
